...
int main(){
 ...
 switch (TRUE){
  ...
  case (!strncmp(cmd, "CD", 2)):
   char *newDIR = cmd + 3;
   if (chdir(newDIR) == -1){
    printf("%s: no such directory\n", newDIR);
   }

  case (!strncmp(cmd, "TCP PORT", 8)):
   printf("Connecting to server!\n");
   toServer();

  case (!strncmp(cmd, "LOCAL", 5)):
   // REDIRECTING BACK THE STDOUT
   dup2(666, 1);
   printf("Back to Shell\n");
   continue;

  case (!strncmp(cmd, "COPY", 4)):
  ...
  case (!strncmp(cmd, "DELETE", 6)):
   char *fileDEL = cmd + 7;
   if (unlink(fileDEL) == -1)
   { // DELETE FUNCTION IF RETURNS -1 COULDNT DELETE
  ...
  default:
   ...
   else if (pid == 0)
   {
    ...
    { // IF -1 THAN BASH COULDNT COMPLETE THE TASK
     printf("Bad command\n");
     break;
    }
   }
